...
class FetchRunLogsTool(BaseTool):
    ...
    def _run(self, run_id: int) -> str:
        ...
        # Step 1: failed jobs ও steps identify করুন
        g = Github(token)
        repo = g.get_repo(repo_name)
        run = repo.get_workflow_run(int(run_id))
        ...
        # Step 2: actual log zip download করুন
        headers = {
        ...
        # Step 3: zip থেকে error lines extract করুন
        error_lines = []
        try:
        ...
        # Step 4: duplicate বাদ দিন, max 50 lines
        seen = set()
        unique_errors = []
